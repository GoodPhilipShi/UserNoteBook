...
let include = [
    {
        ...
        , to: (url) => url.replace("https://github.com/", proxyMap[server].down)
    }
    , {
        match: (url) => url.startsWith("https://raw.githubusercontent.com/") >= 0
        , to: (url) => url.replace("https://raw.githubusercontent.com/", proxyMap[server].raw)
    }
    , {
        match: (url) => url.startsWith("https://github.com/") >= 0
        , to: (url) => url.replace("https://github.com/", proxyMap[server].home)
    }
]
...
module.exports = class ProxyGithub extends Plugin {
    ...
    onload() {
        new window.Notice("欢迎使用 ProxyGithub 代理访问社区插件 ！")
        this.addSettingTab(new ProxyGithubSettingTab(this.app, this));
        ape.regedit();
        apc.regedit();
        app.regedit();
        this.loadData().then(v => {
            this.settings = Object.assign({}, { server: 'mirr' }, v);
        })
    }

    setValue(key, value) {
        this.settings[key] = value
        this.saveData(this.settings).then((v) => {
            new window.Notice(`✔ 切换 ${key} 为 ${value} 成功`)
        }).catch((e) => {
            new window.Notice(`❌ 切换 ${key} 为 ${value} 失败`)
        })
    }
